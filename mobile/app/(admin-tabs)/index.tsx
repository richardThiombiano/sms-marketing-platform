      ...
      >
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <View style={{ width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 12 }}>
              <LinearGradient colors={['#EA580C', '#DC2626']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }}>
                <Ionicons name="shield-checkmark" size={22} color="#FFFFFF" />
              </LinearGradient>
            </View>
            <View>
              <Text style={{ fontSize: 24, fontWeight: '800', color: '#1E293B' }}>Administration</Text>
              <Text style={{ fontSize: 13, color: '#64748B' }}>Bonjour, {user?.first_name || 'Admin'}</Text>
            </View>
          </View>
        </View>
        ...
        {/* Quick Actions */}
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 20, marginBottom: 4 }} contentContainerStyle={{ paddingHorizontal: 20, gap: 10 }}>
          {[
            { label: 'Facturation', icon: 'card-outline' as const, route: '/(screens)/admin/billing' },
            { label: 'Inscriptions', icon: 'person-add-outline' as const, route: '/(screens)/admin/registrations' },
          ].map((action) => (
            <Pressable key={action.label} onPress={() => router.push(action.route as Href)} style={({ pressed }) => ({ paddingHorizontal: 16, paddingVertical: 12, borderRadius: 14, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E2E8F0', flexDirection: 'row', alignItems: 'center', gap: 8, opacity: pressed ? 0.7 : 1, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 })}>
        ...
        <View style={{ paddingHorizontal: 20, marginTop: 24 }}>
          <Text style={{ fontSize: 16, fontWeight: '700', color: '#1E293B', marginBottom: 12 }}>Entreprises récentes</Text>
          {tenants.length === 0 ? (
        ...
        {/* Recent Users */}
        <View style={{ paddingHorizontal: 20, marginTop: 24 }}>
          <Text style={{ fontSize: 16, fontWeight: '700', color: '#1E293B', marginBottom: 12 }}>Utilisateurs récents</Text>
          {users.length === 0 ? (
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 14, padding: 20, alignItems: 'center' }}>
