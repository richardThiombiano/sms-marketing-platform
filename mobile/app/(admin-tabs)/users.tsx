...
function CreateUserModal({ visible, onClose, onCreated }: { visible: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!form.tenant_id || !form.username || !form.email || !form.first_name || !form.password) {
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20 }}>
          ...
          <TextInput value={form.username} onChangeText={(v) => updateForm('username', v.toLowerCase().replace(/[^a-z0-9._]/g, ''))} placeholder="ex: jean.dupont" autoCapitalize="none" placeholderTextColor="#94A3B8" style={{ backgroundColor: '#FFF', borderRadius: 10, borderWidth: 1, borderColor: '#E2E8F0', padding: 12, fontSize: 14, color: '#1E293B', marginBottom: 4 }} />
...
function EditUserModal({ visible, user, onClose, onUpdated }: { visible: boolean; user: any; onClose: () => void; onUpdated: () => void }) {
  const [form, setForm] = useState({ username: '', email: '', first_name: '', last_name: '', role: '', password: '' });
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Ne pas envoyer le mot de passe s'il est vide (pas modifié)
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20 }}>
          ...
          <Text style={{ fontSize: 12, fontWeight: '600', color: '#475569', marginBottom: 6 }}>Username</Text>
          <TextInput value={form.username} onChangeText={(v) => updateForm('username', v.toLowerCase().replace(/[^a-z0-9._]/g, ''))} autoCapitalize="none" placeholderTextColor="#94A3B8" style={{ backgroundColor: '#FFF', borderRadius: 10, borderWidth: 1, borderColor: '#E2E8F0', padding: 12, fontSize: 14, color: '#1E293B', marginBottom: 12 }} />

          <Text style={{ fontSize: 12, fontWeight: '600', color: '#475569', marginBottom: 6 }}>Email *</Text>
          <TextInput value={form.email} onChangeText={(v) => updateForm('email', v)} keyboardType="email-address" autoCapitalize="none" style={{ backgroundColor: '#FFF', borderRadius: 10, borderWidth: 1, borderColor: '#E2E8F0', padding: 12, fontSize: 14, color: '#1E293B', marginBottom: 16 }} />

          <Text style={{ fontSize: 11, fontWeight: '700', color: '#94A3B8', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>Rôle & Sécurité</Text>
          <Text style={{ fontSize: 12, fontWeight: '600', color: '#475569', marginBottom: 6 }}>Rôle</Text>
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 16 }}>
            {['member', 'admin', 'owner'].map((r) => (
              <Pressable key={r} onPress={() => updateForm('role', r)} style={{ flex: 1, paddingVertical: 10, borderRadius: 10, backgroundColor: form.role === r ? '#EA580C' : '#FFF', borderWidth: 1, borderColor: form.role === r ? '#EA580C' : '#E2E8F0', alignItems: 'center' }}>
          ...
          <TextInput value={form.password} onChangeText={(v) => updateForm('password', v)} placeholder="Laisser vide pour ne pas changer" placeholderTextColor="#94A3B8" secureTextEntry style={{ backgroundColor: '#FFF', borderRadius: 10, borderWidth: 1, borderColor: '#E2E8F0', padding: 12, fontSize: 14, color: '#1E293B', marginBottom: 20 }} />
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12 }}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 24, fontWeight: '800', color: '#1E293B' }}>Utilisateurs</Text>
            ...
            <View style={{ backgroundColor: '#FFF', borderRadius: 16, padding: 14, marginBottom: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ fontSize: 11, color: '#64748B' }}>{item.email}</Text>
                </View>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <View style={{ flexDirection: 'row', gap: 6 }}>
                  ...
                  {item.role !== 'superadmin' && (
                    <>
                      <Pressable onPress={() => setEditUser(item)} style={({ pressed }) => ({ opacity: pressed ? 0.5 : 1, padding: 2 })}>
      ...
      <EditUserModal visible={!!editUser} user={editUser} onClose={() => setEditUser(null)} onUpdated={loadUsers} />
    </SafeAreaView>
  );
}
